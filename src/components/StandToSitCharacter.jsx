...
export default function StandToSitCharacter(props) {
    ...
    // Load animations
    const { animations: animationsStandToSit } = useFBX("/assets/animations/StandToSit.fbx");
    const StandToSitAnimation = animationsStandToSit[0];

    // Rename animation for clarity
    StandToSitAnimation.name = "StandToSit";
    ...
    const { actions } = useAnimations([StandToSitAnimation], group);
    ...
    useEffect(() => {
        ...
        stopAllAnimations();
        actions.StandToSit?.reset().play(); // Play the relax animation
    }, [actions]);
